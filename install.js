...
// Exported entry for the CLI wrapper
...
• {yellow-fg}⚠{/yellow-fg} Warnings: ${this.warnings.length}`;
      ...
      try {
        ...
        for (const [name, config] of Object.entries(hydraMCPs)) {
          if (existingConfig.mcpServers[name]) {
            this.log(
              `MCP server '${name}' already configured, skipping`,
              "info"
            );
            skippedCount++;
          } else {
      ...
      try {
        ...
        for (const extra of extras) {
          try {
            ...
            await execAsync(
              `git clone --depth 1 "${extra.repo}" "${targetDir}"`
            );
    ...
    showCompletionSummary(startTime) {
      ...
      this.log(
        `✅ Configuration saved to: ${this.claudeConfigPath}`,
        "success"
      );
...
console.log(
  "[HYDRA-INSTALLER] file comparison:",
  import.meta.url === `file://${process.argv[1]}`
);

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(
    "[HYDRA-INSTALLER] Running as main module, starting installer..."
  );
  ...
  console.log(
    "[HYDRA-INSTALLER] Not running as main module, module loaded for import"
  );
}
